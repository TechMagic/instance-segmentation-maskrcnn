...
import vector.utils.maskrcnn.utils as utils
...
def random_crop_with_content(img, mask, patch_size, min_nr_mask_pixels=0, max_attempts=1000):
    """Take a random crop from the image and mask such that at least min_nr_mask_pixels are 		present in mask. 
        Stop after max_attempts.
    
        img: 3d numpy array (h,w,c)
        mask: 3d numpy array (h,w,n)
        patch_size: [s_h, s_w]
        """
    
    for i in range(max_attempts):
        crop_img, crop_mask = random_crop(img, mask, patch_size)
        nr_pixels = np.sum(crop_mask)
        if nr_pixels >= min_nr_mask_pixels:
            break
...
def random_crop(img, mask, patch_size):
    """Take a random crop from the image and mask.
    ...
    h,w = img.shape[:2]
    p_x = random.randint(0, h-patch_size[0])  #random.randint(0,2) generates 0,1 or 2.
    p_y = random.randint(0, w-patch_size[1])
    
    crop_img = img[p_x:(p_x + patch_size[0]) , p_y:(p_y + patch_size[1]), :]
    crop_mask = mask[p_x:(p_x + patch_size[0]) , p_y:(p_y + patch_size[1]), :]
    
    return crop_img, crop_mask


def rotate_2d_img_and_mask(img, mask, angle):
   
	center = [img.shape[0]/2, img.shape[1]/2]
	
	#rotation matrix
	M = cv2.getRotationMatrix2D(center=(center[1],center[0]), angle=angle, scale=1.0)

	#rotate image
	rotated_image = cv2.warpAffine(img, M, (img.shape[1], img.shape[0]),
		                           flags=cv2.INTER_LINEAR + cv2.WARP_FILL_OUTLIERS, borderValue=(255, 255, 255))

	#rotate mask
	rotated_mask = cv2.warpAffine(mask, M, (mask.shape[1], mask.shape[0]),
		                           flags=cv2.INTER_LINEAR + cv2.WARP_FILL_OUTLIERS, borderValue=(0, 0, 0, 0))

	rotated_mask = (rotated_mask > 0.5).astype('uint8') 

	return rotated_image, rotated_mask
  

def load_image_gt(dataset, config, image_id, augment=False,
                  use_mini_mask=False, verbose=0):
    """Load and return ground truth data for an image (image, mask, bounding boxes).

    augment: If true, apply random image augmentation. Currently, only
        horizontal flipping is offered. --> Added rotation...(hack)
    ...
    #Added: take a random patch
    image, mask = random_crop(image, mask, config.PATCH_SIZE)
    
    #print('after random crop','image shape', image.shape, 'mask shape', mask.shape)
    ...
    if augment:
        ...
        if config.AUGMENT_ROTATE:
            ...
            image, mask = rotate_2d_img_and_mask(image, mask, angle)

 
    #print('after augment','image shape', image.shape, 'mask shape', mask.shape)
    ...
    #print('after remove bbox','image shape', image.shape, 'mask shape', mask.shape)
    ...
    #print('after use_mini_mask','image shape', image.shape, 'mask shape', mask.shape)
    ...
    #print('at end of load_gt','image shape', image.shape, 'mask shape', mask.shape)

    return image, image_meta, class_ids, bbox, mask

    
def load_image_gt_original(dataset, config, image_id, augment=False,
                  use_mini_mask=False, verbose=0):
    """Load and return ground truth data for an image (image, mask, bounding boxes).
...
class MaskRCNN():
    ...
    def __del__(self):
        # JF 2019-02-18 Explicitly clear tensorflow session when object goes out of scope
        # Otherwise results in a massive memory leak!
        K.clear_session()
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path,
                                            verbose=0, save_weights_only=True, period=self.config.SAVE_INTERVAL),
        ]
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window):
        ...
        full_masks = np.stack(full_masks, axis=-1)\
            if full_masks else np.empty( (0,0,0))
